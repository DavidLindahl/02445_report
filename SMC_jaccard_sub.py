...
import re
# Import the data
df_read = pd.read_csv("Prompts/Subjective_prompts.csv")

# Dropping bad data
df_read.drop(['most horrifying crimes in 20th century Europe','most iconic moments in fashion shows'], axis = 1)
...
smc_mean_matrix_sub = np.zeros(n_categories)
jac_mean_matrix_sub = np.zeros(n_categories)
for i in range(n_categories):
    smc_mean_matrix_sub[i] = np.mean(smc_matrix[i,:])
    jac_mean_matrix_sub[i] = np.mean(jac_matrix[i,:])

result_data = {'SMC Mean': smc_mean_matrix_sub, 'JAC Mean': jac_mean_matrix_sub}
results_sub = pd.DataFrame(result_data, index=category_names)

results_sub.to_csv('Similarities/similarities_subjective.csv')
print(results_sub)
print(f"smc_mean_matrix: {smc_mean_matrix_sub}")
print(f"jac_mean_matrix: {jac_mean_matrix_sub}")
